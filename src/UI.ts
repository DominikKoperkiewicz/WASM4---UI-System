...
export class Slider extends UIElement{
    ...
    constructor(x: i16, y: i16, text: string, width: u8, min: i16, max: i16, defaultValue: i16) {
        super();
        this.x = x;
        this.y = y;
        this.text = text.toUpperCase();
    ...
    private draw(x: i16, y: i16) : void {
        ...
        Text.write(this.text + " " + this.value.toString(), x, y-6, 0x40);
    }
    ...
    update(x: i16, y: i16) : void {
        ...
        if(this.decreaseButton.isHold && !this.holdout) { this.value--; }
        if(this.increaseButton.isHold && !this.holdout) { this.value++; }
        ...
        if(this.knobButton.isHold) {
            this.value = <i16>((this.max-this.min) * (<f32>Mouse.x - <f32>x - 12) / <f32>(this.width - 25) + this.min );
        }

        this.value = <i16>Math.max(this.min, Math.min(this.value, this.max));
